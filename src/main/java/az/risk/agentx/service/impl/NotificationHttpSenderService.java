...
//    private static final String API_URL = "https://devappsback.ziraatbank.az:8188/api/CiscoWebHook/%s";
    ...
    public void answeredCall(String sessionId) {
        ...
        try (CloseableHttpClient httpClient = HttpClients.custom()
            ...
            HttpPost httpPost = new HttpPost(API_URL.formatted("AcceptedCall"));
            httpPost.setEntity(new StringEntity(requestBody));
    ...
    public void agentStateChanged(AgentStateEvent event) {
        log.trace("Send Agent state change event : {}", event);
        ...
        try (CloseableHttpClient httpClient = HttpClients.custom()
            ...
            HttpPost httpPost = new HttpPost(API_URL.formatted("ChangedStatus"));
            httpPost.setEntity(new StringEntity(json));
            ...
            try (CloseableHttpResponse response = httpClient.execute(httpPost)) {
                ...
                log.trace("Ziraat ChangedStatus api response code is {}", statusCode);
                log.trace("Ziraat ChangedStatus api response body is {}", responseStr);
                if (statusCode != 200) {
